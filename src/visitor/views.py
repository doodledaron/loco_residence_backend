...
def register_visitor(request):
    ...
    check_in_date = request.data.get('check_in_date')  # Date part only
    ...
    # we need to parse the check_in_date string to a datetime object
    ...
    check_in_date_only = check_in_datetime.date()  # Get the date part
 
    # Check if a visitor with the same details already exists
    duplicate_visitor = Visitor.objects.filter(
        full_name=full_name,
        hp_number=hp_number,
        car_plate_no=car_plate_no,
        check_in_date=check_in_date_only,  # Use the date part only
        purpose_of_visit=purpose_of_visit
    ).exists()

    if duplicate_visitor:
        return Response({'error': 'Visitor with these details already exists'}, status=status.HTTP_400_BAD_REQUEST)

    # Create a new Visitor object with date and time stored separately
    visitor = Visitor(
        full_name=full_name,
        hp_number=hp_number,
        car_plate_no=car_plate_no,
        check_in_date=check_in_date_only,  # Store date part
