...
class ResidentParking(models.Model):
    ...
    parking_id = models.ForeignKey(Parking, on_delete=models.CASCADE, related_name='resident_parkings')
    resident_id = models.ForeignKey(Resident, on_delete=models.CASCADE, related_name='resident_parkings')
    is_occupied = models.BooleanField(default=False)
    
class VisitorParking(models.Model):
    slot_no = models.CharField(max_length=100)
    parking_id = models.ForeignKey(Parking, on_delete=models.CASCADE, related_name='parking')
    is_occupied = models.BooleanField(default=False)
